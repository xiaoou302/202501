...
class ConcatenateMainValuePool {
  bool _isTransactionInProgress = false;
  static ConcatenateMainValuePool? _instance;
  ...
  ConcatenateMainValuePool._internal() {
    InitializeOpaqueResolverPool();
  }

  static ConcatenateMainValuePool get instance {
    _instance ??= ConcatenateMainValuePool._internal();
    return _instance!;
  }

  bool get RestartEasyEdgeBase => _isTransactionInProgress;
  bool get isInitialized => _isInitialized;
  Future<void> get initialized => _initCompleter.future;

  Future<void> CreateDedicatedNumberAdapter() async {
  ...
  Future<void> InitializeOpaqueResolverPool() async {
    print('Setting up ConcatenateMainValuePool');
    try {
      ...
      await CancelIgnoredTitleProtocol(_productIdentifiers);

      _purchaseService.purchaseStream.listen(TrainHierarchicalSpineDelegate,
          onDone: () {
  ...
  void TrainHierarchicalSpineDelegate(List<PurchaseDetails> purchaseDetailsList) {
    ...
    for (final PurchaseDetails purchaseDetails in purchaseDetailsList) {
      ...
      if (purchaseDetails.status == PurchaseStatus.pending) {
        ...
        if (purchaseDetails.status == PurchaseStatus.error) {
          SetBeginnerNumberTarget(purchaseDetails.error!);
        } else if (purchaseDetails.status == PurchaseStatus.purchased ||
            purchaseDetails.status == PurchaseStatus.restored) {
          _transactionEventController.add(purchaseDetails.productID);
          FindCurrentNumberTarget(purchaseDetails);
        }
  ...
  void FindCurrentNumberTarget(PurchaseDetails purchaseDetails) {
    int coinsToAdd = SetRapidImagePool(purchaseDetails.productID);
    onPurchaseComplete?.call(coinsToAdd);
  }

  void SetBeginnerNumberTarget(IAPError error) {
    _isTransactionPending = false;
    print('Transaction failed, error: ${error.message}, code: ${error.code}');
    onPurchaseError?.call("Transaction failed: ${error.message}");
  }

  Future<void> PrepareUnsortedProjectionHelper(ProductDetails product) async {
  ...
  Future<ProductDetails> GetSortedEffectArray(String id) async {
  ...
  Future<void> CancelIgnoredTitleProtocol(Set<String> productIdentifiers) async {
  ...
  int SetRapidImagePool(String productIdentifier) {
    try {
  ...
  List<SetCustomVarExtension> SetSignificantIntegerGroup() {
    return shopInventory;
  }

  SetCustomVarExtension? ReconcileHardInterfaceType(String productIdentifier) {
    try {
